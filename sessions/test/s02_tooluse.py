...
def tool_read_file(file_path: str) -> str:
    """读取文件内容"""
    print_tool("read-file", file_path)
    try:
...
def agent_loop() -> None:
    ...
    while True:
        ...
        # 所以使用 while true 循环，直到 stop_reason != "tool_use"
        while True:
            try:
            ...
            if response.stop_reason == "end_turn":
                ...
                for block in response.content:
                    if hasattr(block, "text"):
                        assistent_text += block.text
                if assistent_text:
                    print_assistant(assistent_text)
                # 跳出内循环，等待下一次用户输入
                break
